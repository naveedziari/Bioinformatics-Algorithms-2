...
#group members: Alex, Lucas, Aiden, Erik, Max
...
class MinimizerIndexer(object):
    ...
    def __init__(self, targetString, w, k, t):
        ...
        for i in range(len(targetString)-w+1):
            current_window = targetString[i:i+w]
            minmer_list = [current_window[j:j+k] for j in range(w-k+1)]
            minmer_list.sort() #lexicographical sort
            minmer = minmer_list[0] 
            for q in range(len(current_window)): #see if the sorted minmer is the first one in the current window, if so, add to dictionary storing indices
                if current_window[q:q+k] == minmer: 
                    index = i + q
                    if minmer in self.minimizerMap:
                        if index in self.minimizerMap[minmer]: pass
                        else: self.minimizerMap[minmer] = self.minimizerMap[minmer] + (index,)
                    else: self.minimizerMap[minmer] = (index,)
                    break
    ...
    def getMatches(self, searchString):
        ...
        for i in range(len(searchString)-k+1):
            kmer = searchString[i:(i+k)]
...
class SeedCluster:
    ...
    def clusterSeeds(seeds, l):
        ...
        This clustering method does not take into account order nor orientation.
        Imagine there is a distance function that measures how far the seeds are from each other, enumerate the components and group by distance (with diagonals representing seeds)
        If the middle of two seeds is less than some threshold, consider to be close.
        Dynamic programming algorithms that build optimal alignments from parts and extend globally will consider order. 

        imagine you have this distance function that measures how far they are. then literally enumerate components. 
        grouping them by distance
        diagonals represent seeds
        if the middle of the two seeds is less than some threshold, count as being close. 
        clique -> enumerate and call as component
        many seeds cannot be simultaneously true because 
        does not take into account order of seeds - other methods like dynamic programming use seeds 
        it's just enumerating connected components
        tell me about whether or not the sets of seeds you got illuminate the true alignment
        """

        # Code to complete - you are free to define other functions as you like
 
        #extract all seeds into a list
        seed_list = []
        seed_components = {}
        for i in range(len(seeds)):
            if seeds[i][1]:
                for j in seeds[i][1]:
                    ...
                    seed_components[(seeds[i][0],j)] = [(seeds[i][0],j)] #use dictionary in order to update with clustering
        ...
        for i in range(len(component_list)):
            ...
            for j in range(i+1,len(component_list)):
                ...
                if abs(x1-x2) <= l and abs(y1-y2) <= l: #distance metric threshold
...
class SmithWaterman(object):
    def __init__(self, string1, string2, gapScore=-2, matchScore=3, mismatchScore=-3):
        ...
        Alternative strategies include 'banded alignment' and 'x-drop'.
        Efficiency increase tactics: instead of computing area of matrix, compute a thin slither of it (unless the real alignment walks another way). 
        Then stop enumerating over some threshold.         
        """
        ...
        for i in range(1, len(string1) + 1):
            for j in range(1, len(string2) + 1):
                ...
                insert_1 = self.editMatrix[i-1][j] + gapScore #insert in string1
                insert_2 = self.editMatrix[i][j-1] + gapScore #insert in string2
                self.editMatrix[i][j] = max(0, diagonal, insert_1, insert_2)

                #use conditionals outlined in docstring of getAlignment to determine pointers for tracebackMatrix
                if string1[i-1] == string2[j-1]:
    ...
    def getAlignment(self):
        ...
        #start at highest-scoring index
        ...
        while self.editMatrix[i][j] != 0:
            ...
            if self.tracebackMatrix[i][j] == 'd':
                ...
                i -= 1
                j -= 1
            #if insert in string1
            elif self.tracebackMatrix[i][j] == 'v':
                i -= 1
            #if insert in string2
            elif self.tracebackMatrix[i][j] == 'h':
                j -= 1

        return alignedPairs[::-1] #reverse from beginning of alignment 
    ...
    def getMaxAlignmentScore(self):
        ...
        """
        # Code to complete
        return numpy.amax(self.editMatrix) #max value in score matrix
...
def simpleMap(targetString, minimizerIndex, queryString, config):
    ...
    Faster implementation of above methods with the suggestions in questions 1 & 2 would certainly speed up the execution of the code below. 
    In addition, one can dynamically program the simpleMap such that it avoids a loop and builds the table upon its sub-mappings in each iteration
    """
    bestAlignment = [None]
